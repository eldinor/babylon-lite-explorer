...
- Texture UV fields and `invertY` remain read-only in the default adapter. Babylon Lite 1.3.0 documents them as build-time fields: changing them after pipeline compilation requires a material rebuild, but no general public rebuild operation is exposed by the API used here.
