# Babylon Lite 1.11.0 public API inventory

Audited on 2026-07-17 against the installed `@babylonjs/lite@1.11.0` declarations and package implementation, plus the default Explorer adapter. Babylon Lite exposes one public package entry point: `@babylonjs/lite`.
...
| Explorer feature | Babylon Lite 1.11.0 public surface | Explorer behavior |
| --- | --- | --- |
...
| Materials | `Mesh.material`, `Material`, `MaterialView`, `getMaterialFamily()`, family type guards, PBR/Standard public properties, `metadata` | Derives materials from meshes, deduplicates by identity, identifies official PBR, Standard, Node, and Shader families, preserves material-view labels, edits the documented values listed below, and displays metadata read-only. Structural detection remains as a compatibility fallback. |
...
Babylon Lite 1.11.0's `getMaterialFamily()` and `isPbrMaterial()` identify an empty `createPbrMaterial()` result without relying on property-shape heuristics.
...
## Public 1.11.0 surfaces not currently shown
...
These are supported by public 1.11.0 APIs and fit the existing adapter without private-state access:
...
Node Particle APIs are public in Lite 1.11.0, but registered particle systems are not exposed through a public scene collection. A host-retained `NodeParticleSet` or `ParticleSystem` can be supplied through a registered/composed adapter.
